...
export type RelatedListenersMap = Map<ViewModelCore, {
...
export function parsePropertyPath(propertyPath: string | PropertyPathItem | PropertyPathItem[]): PropertyPathItem[] {
  return isString(propertyPath as unknown) ? (
    (propertyPath as unknown as string).indexOf('.') > 0 ? (propertyPath as unknown as string).split('.') : [propertyPath as unknown as PropertyPathItem]
  ) : (
    isArray(propertyPath) ? propertyPath as unknown as PropertyPathItem[] : [propertyPath as PropertyPathItem]
  );
}
 ...
 * @internal
 */
export function shiftParent(child: ViewModelCore, parent: ViewModelCore, property: string | number, delta: number): void {
