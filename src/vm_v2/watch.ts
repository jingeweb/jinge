...
export function watch<T extends object, V = any>(
  vm: T,
  propertyPath: PropertyPathItem[],
  handler: WatchHandler<V>,
  options?: WatchOptions,
): UnwatchFn;
...
function handleVmChange(vmCore: ViewModelCore, changedPath?: PropertyPathItem[]) {
  ...
  watchers.forEach((watcher) => {
    ...
    let match =  clen === 0 || (clen <= watchPath.length && changedPath && !changedPath.some((v, i) => v !== watchPath[i]));
    if (!match && deep) {
      // 如果是深度 watch，且监听 watchPath 是发生变化的 changedPath 的前缀，说明发生变化的是深度监听对象的子元素，需要触发 listener
      if (clen > watchPath.length && changedPath && !watchPath.some((v, i) => changedPath[i] !== v)) {
        match = true
      }
    }
    if (!match) {
