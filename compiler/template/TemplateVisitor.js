...
class TemplateVisitor extends TemplateParserVisitor {
  ...
  _logParseError(tokenPosition, msg, type = 'Error') {
    ...
    this._webpackLoaderContext.emitError(
      new Error(`${type} occur at line ${tokenPosition.line + this._baseLinePosition - 1}, column ${
...
> ${msg}`),
    ...
    attrCtxs.forEach((attrCtx) => {
      ...
      if (a_category === 'vm-use') {
        ...
        if (vms.find((v) => v.name === aval))
          this._throwParseError(attrCtx.start, 'vm-use type attribute name dulipcated: ' + a_name);
      ...
      if (a_category === 'vm-pass') {
        ...
        if (vmPass.find((v) => v.name === a_name))
          this._throwParseError(attrCtx.start, 'vm-pass type attribute name dulipcated: ' + a_name);
        vmPass.push({
          name: a_name,
          expr: aval,
        });
  ...
  _parse_html_ele(etag, ctx) {
    ...
    const elements = this._visit_child_nodes(ctx, result.vms, {
      type: 'html',
      isSVG: this._parent.isSVG || etag === 'svg',
    });
    const setRefCode = result.ref
      ? this._replace_tpl(TPL.SET_REF_ELE, {
          NAME: result.ref,
        })
      : '';
...
return el;`,
    ...
    if (result.argUse) {
      return this._parse_arg_use_parameter([rtnEl], result.argUse, result.vmPass, vmLevel);
    }
    if (result.argPass) {
      return {
        type: 'component',
  ...
  _parse_translate(ctx) {
    ...
    const registerI18N = (renderOfContentNodes) => {
      [
        ...
        new RegExp(`[\\w\\d$_]+${this._importPostfix}`, 'g'),
      ].forEach((reg) => {
      ...
      (locale, contentOrNodes) => {
        if (typeof contentOrNodes === 'string') {
          if (!contentOrNodes) {
            ...
            if (err) {
              this._webpackLoaderContext.emitError(
                new Error(`i18n parse error locale "${locale}" at ${this._resourcePath}`),
              );
              contentOrNodes = defaultLocaleContentNodes;
            } else {
        ...
        let renderOfContentNodes = contentOrNodes
          .map((n) => {
            return this.visitHtmlNode(n);
          })
          .filter((el) => !!el)
...
${prependTab(renderOfContentNodes, true, 4)}
  ...
  _parse_component_ele(tag, Component, ctx) {
    ...
    if (tag === '_slot' && result.argUse) {
      return this._parse_arg_use_parameter(elements, result.argUse, result.vmPass, vmLevel);
    }
...
${
  result.listeners.length > 0
    ? this._prependTab(`  listeners: {
${result.listeners.map(
  (lt) =>
    `    ${attrN(lt[0])}: { fn: function(...args) { ${lt[1].code} }, opts: ${
    ...
    if (computedMemberExprs.length === 0) {
      if (levelPath.length === 1) {
        ...
        return [
          ...
          }${escodegen.generate(expr)}${needWrapViewModel ? ')' : ''}$RENDER_END$\n};`,
          'fn_$ROOT_INDEX$();',
          '',
          `${watchPaths
...
function _notify_${lv_id}() {
  ...
  $RENDER_START$${needWrapViewModel ? `vm${sharedOptions.symbolPostfix}(` : ''}${escodegen.generate(expr)}${
          needWrapViewModel ? ')' : ''
        }$RENDER_END$
}`);
      ...
      if (cn.ruleIndex === TemplateParser.RULE_htmlText) {
        ...
        eles.push(
          this._parent.type === 'html'
            ? txt
            : this._replace_tpl(TPL.TEXT_CONST, {
                VAL: txt,
              }),
        );
      } else {
        ...
        eles.push(
          this._replace_tpl(TPL.TEXT_EXPR, {
